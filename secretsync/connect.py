"""Native cognis-connect emit for secretsync — forward findings to any platform.

Maps secretsync's JSON output to the canonical `Finding` and forwards it via
...
Usage:
    secretsync ... --format json | secretsync-emit --to stix
    secretsync-emit --to slack --url $WEBHOOK --dry-run < findings.json
"""
...
def map_record(rec: dict) -> dict:
    ...
    try:
        out = dict(rec)
        out.pop('value', None)
        out['title'] = rec.get('name', '') + ' Secret'
        out['severity'] = 'low'
        out['type'] = 'secret'
        out['description'] = f"Sealed secret for {rec.get('name', '')}"
        out['tags'] = ['secretsync']
